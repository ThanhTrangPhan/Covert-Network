...
void wait_IAT_before_send(u_int32_t *IAT_array, int IAT_array_elements)
{
...
int main(int argc, char *argv[])
{
	...
	int arg_c_set = 0;
	int quiet = 0;
	int dummy_array[10000] = {'\0'};
	int dummy_array_elements = 0;
	int dummy_set = 0;
	...
	while ((ch = getopt(argc, argv, "vhP:D:d:i:o:u:p:x:c:s:t:T:q")) != -1)
	{
		...
		case 'u':
...

			{
				...
				memset(dummy_array, 0, dummy_array_elements * sizeof(dummy_array[0]));
				...
				for (ptr_optarg = optarg;; ptr_optarg = NULL)
				{
					...
					if (atoi(token) < 0 && atoi(token) >= 0xff)
					{
						fprintf(stderr, "Dummy field value out of range.\n");
						exit(ERR_EXIT);
					}
					dummy_array[dummy_array_elements] = atoi(token);
					dummy_array_elements++;
					if (dummy_array[dummy_array_elements] > 255 || dummy_array[dummy_array_elements] < 0)
					{
						fprintf(stderr, "Exceed values given in -t\n");
						exit(ERR_EXIT);
					} /*else {
						fprintf(stderr, "IAT_array_elements=%d <= %d\n",
							IAT_array_elements, MAX_NUM_PREDEF_IATS);
			...
			if (dummy_array_elements > 0)
			{
				dummy_set = 1;
				if (!arg_c_set)
				{
					num_of_pkts_to_send = dummy_array_elements;
				}
			}
			break;
		case 'o':
	...
	if (!ret_val)
	{
		if (ret_val == 0)
		{
			fprintf(stderr, "Destination address not a valid character string.\n");
			exit(ERR_EXIT);
		}
		else
		{
	...
	/* now override 1-8 bytes of the padding (so that padding remains, if necessary) */
	...
	{
		hdr.dummy = dummy_array[cnt];
		hdr.sequence_number = sequence_number+cnt;
